...
Here, we suppose that the data have been pre-processed (quality control and imputation), and is available as a PLINK binary file containing 15,250 individuals and 281,122 SNPs.
...
all.equal(gwas2$P, 10^predict(gwas)) 
```
...
```{r}
# Cleaning
file.remove(list.files(pattern = "plink*|*.txt"))
```
