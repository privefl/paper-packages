...
```{r}
system.time(
  test <- snp_fastImpute(GNA, infos.chr = popres$map$chromosome, ncores = NCORES)
...
data_frame(col = indNA[, 2], error = XNA[indNA] != X[indNA]) %>%
  ...
  mutate(nb_err_est = (test$pError * test$pNA * n)[col]) %>%
  ggplot() %>%
  bigstatsr:::MY_THEME(coeff = 1.8) + 
...
```{r}
system.time(
  bedfile2 <- snp_beagleImpute(beagle.path = "../../beagle.21Jan17.6cc.jar",
                               plink.path = "../plink_linux_x86_64/plink",
