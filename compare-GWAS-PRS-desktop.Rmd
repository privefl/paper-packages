...
scores2 <- matrix(NA_real_, length(ind.test), length(thrs))
nb.pred2 <- numeric(length(thrs))
for (i in seq_along(thrs)) {
  tmp <- fread2(glue("plink.S{i}.profile"))
  scores2[, i] <- tmp$SCORESUM
  nb.pred2[i]  <- tmp$CNT[[1]] / 2
}

#AUC
aucs2 <- apply(scores2, 2, AUC, target = y01[ind.test])

bind_rows(
  data_frame(nb = nb.pred,  auc = aucs,  Software = "bigsnpr"),
  data_frame(nb = nb.pred2, auc = aucs2, Software = "PLINK")
) %>%
  ggplot() %>%
  bigstatsr:::MY_THEME() +
  geom_point(aes(nb, auc, color = Software, shape = Software), size = 5) +
  geom_line(aes(nb, auc, color = Software, linetype = Software), show.legend = FALSE) +
  scale_x_log10() + 
  scale_colour_manual(
    values = c("black", "red"),
